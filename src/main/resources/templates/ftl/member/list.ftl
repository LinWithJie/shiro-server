						...
						<#if page?exists && page.list?size gt 0 >
							<#list page.list as it>
								<tr>
									...
									<td>${(it.state==1)?string('有效','禁止')}</td>
									<td>${it.cardMoney?string('0')}</td>
                                    <td>${it.total?string('0')}</td>
									<td>
										<@shiro.hasPermission name="/member/forbidUserById">
										${(it.state==1)?string('<i class="glyphicon glyphicon-eye-close"></i>&nbsp;','<i class="glyphicon glyphicon-eye-open"></i>&nbsp;')}
										<a href="javascript:forbidUserById(${(it.state==1)?string(0,1)},${it.id})">
											${(it.state==1)?string('禁止登录','激活登录')}
                                        </a>
										</@shiro.hasPermission>
										<@shiro.hasPermission name="/member/deleteUserById">
